...
function handleUploadFile(payload, sessionUser) {
  // Generic permission - any authenticated user can upload
  if (!sessionUser) {
...
function handleGetFiles(payload, sessionUser) {
  // Generic permission - any authenticated user can view files
  if (!sessionUser) {
  ...
  try {
    ...
    return {
      ...
      action: 'getFiles',
      data: null,
      message: 'Error retrieving files: ' + error.toString()
    };
  }
}
...
function handleDeleteFile(payload, sessionUser) {
  // Generic permission - any authenticated user can delete their uploads
  if (!sessionUser) {
