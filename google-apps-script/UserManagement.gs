...
function handleGetUsers(payload, sessionUser) {
  ...
  try {
    ...
    return {
      ...
      action: 'getUsers',
      data: null,
      message: 'Error retrieving users: ' + error.toString()
    };
  }
}
...
function handleAddUser(payload, sessionUser) {
  ...
  try {
    ...
    const newUser = {
      ...
      passwordHash: payload.password, // In production, hash this
      role: payload.role,
      status: 'Active',
...
function handleResetPassword(payload, sessionUser) {
  ...
  try {
    ...
    // Update password
    user.passwordHash = payload.newPassword; // In production, hash this
    updateRecord(SHEETS.USERS, payload.id, user);
    ...
    return {
      ...
      action: 'resetPassword',
      data: null,
      message: 'Error resetting password: ' + error.toString()
    };
  }
}
